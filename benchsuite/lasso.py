...
class LassoBenchmark(Benchmark):
    ...
    def __call__(
        ...
        return torch.tensor(
            self._b.evaluate(x.cpu().numpy().astype(np.double)), device=settings.DEVICE,
...
class LassoDNA(LassoBenchmark):
    ...
    def __init__(
        ...
        super().__init__(
            dim=180, lb=torch.ones(180, device=settings.DEVICE, dtype=settings.DTYPE) * (-1),
...
class LassoDNANoisy(LassoBenchmark):
    ...
    def __init__(
        ...
        self._b: LassoBench.RealBenchmark = LassoBench.RealBenchmark(
            pick_data="dna", mf_opt="discrete_fidelity", seed=random_seed
        )
