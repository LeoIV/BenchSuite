...
class MPDRoverBenchmark(Benchmark):
    def __init__(self):
        
        super().__init__(
            ub=torch.tensor(-3 * np.ones(200), dtype=settings.DTYPE, device=settings.DEVICE),
            lb=torch.tensor(3 * np.ones(200), dtype=settings.DTYPE, device=settings.DEVICE),
            dim=200
        )
    def __call__(self, x):
